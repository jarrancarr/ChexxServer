	...
	"Pawn": {
		Step: []Step{
			...
			{Title: "Pawn", Width: 70, PosX: 50, PosY: 30, Background: "#20a0a070",
				...
				Text: []string{"If Chess is familiar to you, then this will be as you might expect.",
					"h2:::A pawn moves forward one space at a time, although, their attacks are only to the forward left and right flank.",
					"On <d1>, highlighted in blue, is a pawn. The green space ahead of him, is where he can move to if that space is unoccupied.  In red, are the spaces to where this pawn can attack an enemy piece.  Only when an enemy piece is present may this pawn take and accupy this space."}},

			{Title: "", Width: 70, PosX: 50, PosY: 40, Background: "#20a0a070",
				Black: []string{}, White: []string{"Pc22", "Pd21"},
				Text: []string{"h2:::Select either pawn with a single click."},
				Answer: [][]string{
					{"d21", "The <d21> pawn is on a starting pawn position,||so there is an option to charge forward 2 spaces.", "15,40,4,#6F6,#50505050", "hl|#0f0|d11 e"},
					{"c22", "This <c22> pawn is on a starting pawn position,||so there is an option to charge forward 2 spaces.", "15,40,4,#6F6,#50505050", "hl|#0f0|c11 c"}},
				Hint: []string{"Click once on either pawn.||35,80,3,#0f0,#000"},
			},
			{Title: "", Width: 70, PosX: 50, PosY: 40, Background: "#20a0a070",
			...
			{Title: "", Width: 70, PosX: 50, PosY: 40, Background: "#20a0a070",
				...
				Text: []string{"h2:::Here are all of White's pawns on their staring positions.", "From any of these positions, a pawn may begin his advance with a 2 spaces charge."},
			},
	...
	"Spearman": {
		Step: []Step{
			...
			{Title: "", Width: 70, PosX: 50, PosY: 35, Background: "#20f02050",
				...
				Answer: [][]string{{"", "", ""},
					{"c * e", "Why are you touching me    ||in my 'no no' places.", "12,48,4,#000,#bbbb8080"},
					{"d2", "", "", "~"},
					{"d2~d", "Tell my family... I died with honor.", "22,40,4,#ff6,#3333bb80", "-"},
					{"d2~d1", "Black's pawns are defenseless against me.   ", "18,40,4,#ff6,#3333bb80", "-||hl|#f00|c e"},
				},
			},
			...
			{Title: "", Width: 40, PosX: 25, PosY: 55, Background: "#20a02090",
				...
				Text:  []string{"h2:::The Spearman formation look like this.", "Quite a wedge, right?  And quite a lot of open space."}},
			...
			{Title: "", Width: 70, PosX: 50, PosY: 30, Background: "#20a02090",
				...
				Text:  []string{"h2:::Both the pawn and spearman are able to attack the Rook.  What is the best attack?"},
				Answer: [][]string{{"", "", "", ""},
					{"e31", "I hear you are a Mandrake mymmerkin.", "22,40,4,#ff6,#222222a0"},
					{"d55 d43", "", "", "~"},
					{"d55~e4", "That... accomplished nothing.   ", "22,43,4,#04b,#dd8833a0", "-"},
					{"d43~e2", "Sacraficed your spearmen?   ", "22,43,4,#04b,#dd8833a0", "-"},
					{"d55~e41", "You pawn attacketh...||The Rook, taketh away.   ", "35,43,4,#04b,#dd8833a0", "-"},
					{"d43~d33", "Good, Rooks are vulnerable to spearmen.   ||See how the Rook can't take?", "20,43,4,#ff6,#3333bb80", "-"},
				},
			},
		},
	},
	...
	"Bishop": {
		Step: []Step{
			...
			{Title: "Knowledge Check", Width: 70, PosX: 50, PosY: 20, Background: "#500060a0",
				...
				Answer: [][]string{{"", "Sorry, but that is incorrect.", "16,48,6,#f80,#00000080"},
					{"d54 e1 e22 c22 c43", "Outstanding work.||Thats a lot of options.", "50,55,4,#8f0,#20202050", "+", "5"},
			...
			{Title: "Knowledge Check", Width: 70, PosX: 50, PosY: 20, Background: "#500060a0",
				...
				Answer: [][]string{{"", "Sorry, but that is incorrect.", "16,48,6,#f80,#00000080"},
					{"d4 d43 f31 c21", "Outstanding work.  You found all||options for a safe retreat.", "50,55,4,#8f0,#20202050", "+", "4|B|#0ff"},
			...
			{Title: "Knowledge Check", Width: 70, PosX: 50, PosY: 20, Background: "#500060a0",
				...
				Black:  []string{"Pf", "Pb1"},
				White:  []string{"Bd3", "Bd2"},
				Text:   []string{"h2:::Blocking scenario. TBD..."},
				Answer: [][]string{{"", "Sorry, but that is incorrect.", "16,48,6,#f80,#00000080"},
					{"d54 e1 e22 c22 c43", "Outstanding work.||Thats a lot of options.", "50,55,4,#8f0,#20202050", "+", "5"},
					{"d3", "Just point me in the right direction.   ", "15,58,4,#f80,#00000080"},
					{"d2", "Standing fast, my lord.  ", "15,58,6,#f80,#00000080"},
			...
			{Title: "", Width: 70, PosX: 50, PosY: 30, Background: "#500060a0",
				...
				Black:  []string{"Kc52"},
				White:  []string{"Bc22", "Bc32", "Bc33"},
				Text:   []string{"h2:::Add the third Bishop, and the King is all but finished."},
			},
			{Title: "Knowledge Check", Width: 50, PosX: 30, PosY: 85, Background: "#500060a0",
				...
				Answer: [][]string{{"", "Sorry, but that is incorrect.", "16,58,6,#f80,#00000080", "-"},
					...
					{"f1~b1", "Oh no, Knight spoils your fun.", "15,44,4,#4f0,#0000a080", "-||hl|#f40|a43"},
					{"f1~d1", "Oh no, Knight spoils your fun.", "15,44,4,#4f0,#0000a080", "-||hl|#f40|b11"},
					{"c53", "I don't do that king of thing.   ", "15,48,4,#f80,#00000080"},
					{"a54 a53 a43 a44", "Piss off, abrigant!  ", "45,58,5,#f80,#00000080"}},
			...
			{Title: "Knowedge Check", Width: 65, PosX: 50, PosY: 25, Background: "#500060a0",
				Black: []string{"Kf42", "Nb", "Nc31"},
				White: []string{"Kd3", "Be1", "Pc4"},
				Text:  []string{"h2:::Not all forks are created equal, though.  What are the troublesome spaces for the Bishop?"},
	...
	"Rook": {
		Step: []Step{
			...
			{Title: "", Width: 60, PosX: 40, PosY: 15, Background: "#f0f0f050",
				Black: []string{"Nc5", "Be5"},
				White: []string{"Rc2", "Re3"},
				Text:  []string{"Lets look at the Rook in action!", "h2:::Obviously, using the Rook... turn the tides of battle!"}},
		},
	},
	"Archer": {
		Step: []Step{
